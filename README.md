...
3. [Role-aware SAAMR quickstart](./examples_system/Role_Aware_SAAMR_Quickstart.ipynb)  
Builds a role-aware `UNIVERSE -> SEGMENT -> RESIDUE -> PARTICLE` MuPT system from SMILES strings, exports one RDKit molecule per segment, and writes git-ignored SDF files for downstream workflows  
4. [Role-aware SAAMR OpenFF/OpenMM workflow](./examples_system/Role_Aware_SAAMR_OpenFF_OpenMM.ipynb)  
Loads SDF files generated by the role-aware quickstart and demonstrates the dependency-aware OpenFF/OpenMM handoff, with optional template cells for GROMACS and LAMMPS export
5. [Role-aware all-atom PE/PEAA ionomer workflow](./examples_system/Role_Aware_SAAMR_All_Atom_Ionomer_OpenMM.ipynb)  
Builds configurable all-atom PE/PEAA ionomer systems with explicit hydrogen caps and sodium counterions, exports role-aware SDF files, and demonstrates OpenFF/OpenMM compaction with production-start coordinate exports
